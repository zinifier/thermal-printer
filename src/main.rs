        ...
        match &self.image {
            Image::None => {
                content = content.push(
                    widget::button::text("Load image").on_press(Action::SelectImage)
                );
            },
            Image::Errored { path, error } => {
                content = content.push(
                    widget::text(format!("Failed to load {} due to error: {}", path.display(), error))
                );
                content = content.push(
                    widget::button::text("Load image").on_press(Action::SelectImage)
                );
            ...
            Image::Loaded { path, data } => {
                content = content.push(
                    widget::button::text("Load new image").on_press(Action::SelectImage)
                ).push(
                    widget::text(format!("{}", path.display()))
                ).push(
                    widget::row()
                        .push(
                            icon_button(ROTATE_LEFT, 40.0).on_press(Action::Rotate(Direction::Left))
                        ).push(
                            icon_button(ROTATE_RIGHT, 40.0).on_press(Action::Rotate(Direction::Right))
                        )
                ).push(
                    widget::container(
                    ...
                    ).width(Length::Fixed(720.0)).align_x(Alignment::Center).padding(5).style(|_| Color::WHITE.into())
                );
            }
        }
